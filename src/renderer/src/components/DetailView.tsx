...
import { DEFAULT_TOKENS, type Item, type TabData } from '../../../shared/types'
import {
  closeDetail,
  saveOpenItem,
  updateOpenItem,
...
export default function DetailView({ tab, item }: { tab: TabData; item: Item }): React.ReactNode {
  ...
  useEffect(() => {
    ...
    setChat([])
    setChatInput('')
    void window.api.loadChat(tab.dir, item.id).then((entries) => {
  ...
  const sendChat = async (): Promise<void> => {
    ...
    try {
      ...
      setChat(full)
      await window.api.saveChat(tab.dir, item.id, full)
    } catch (e) {
  ...
  const clearChat = async (): Promise<void> => {
    setChat([])
    await window.api.saveChat(tab.dir, item.id, [])
  }
    ...
    <div className="detail-view" onDropCapture={onDropCapture} onDragOverCapture={onDragOverCapture}>
      ...
      {aiConfigured && adviceEnabled && !advice && !thisAdviceBusy && (
        <div className="advice-empty">
          <Button type="text" size="small" icon={<RobotOutlined />} onClick={() => void regenerateAdvice()}>
            AI 도움말 생성
          </Button>
        </div>
      )}
      ...
      {aiConfigured && (
        <div className="chat-panel">
          ...
          {chatBusy && <div className="chat-msg chat-assistant"><div className="chat-content">답변 작성 중…</div></div>}
          <div className="chat-input-row">
            <Input.TextArea
              ...
              disabled={chatBusy}
            />
            <Button type="primary" loading={chatBusy} disabled={!chatInput.trim()} onClick={() => void sendChat()}>
              질문
            </Button>
            {chat.length > 0 && (
