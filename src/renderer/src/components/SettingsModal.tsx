...
export default function SettingsModal({
  ...
  const addTab = async (): Promise<void> => {
    ...
    if (!folder) {
      message.error(
        "폴더명으로 쓸 수 있는 영문/숫자가 이름에 포함되어야 합니다",
      );
      return;
    }
    try {
          ...
          <>
            <Form.Item label="테마">
              <Segmented
                ...
                onChange={(v) =>
                  void saveSettingsPatch({ theme: v as "light" | "dark" })
                }
                options={[
            ...
            <Form.Item label="에디터 글자 크기 (px)" help="본문 기준 크기 — 제목 등은 비율대로 함께 조절됩니다">
              <InputNumber
                min={12}
                max={24}
            ...
            <Form.Item label="자동저장 대기 시간 (분)">
              <InputNumber
                ...
                onChange={(v) =>
                  v && void saveSettingsPatch({ autosave_minutes: v })
                }
              />
            </Form.Item>
