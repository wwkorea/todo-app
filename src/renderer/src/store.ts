...
export async function newItem(tabDir: string): Promise<void> {
  ...
  await window.api.saveItem(tabDir, item, get().settings.backup_keep)
  await window.api.saveTabSetting(tabDir, setting)
}
...
export async function saveOpenItem(): Promise<void> {
  ...
  try {
    await window.api.saveItem(found.tab.dir, found.item, get().settings.backup_keep)
...
export async function updateItemInline(
  ...
  if (saved) await window.api.saveItem(tabDir, saved, get().settings.backup_keep)
}
...
export async function deleteItemAction(tabDir: string, id: string): Promise<void> {
  ...
  await window.api.deleteItem(tabDir, id, get().settings.backup_keep)
  await window.api.saveTabSetting(tabDir, setting)
}
...
export async function moveItemToTab(fromDir: string, id: string, toDir: string): Promise<void> {
  ...
  await window.api.saveItem(toDir, moved, keep)
  await window.api.deleteItem(fromDir, id, keep)
  await window.api.saveTabSetting(toDir, toSetting)
  await window.api.saveTabSetting(fromDir, fromSetting)
}
...
export async function createTabAction(folder: string, name: string, type: 'todo' | 'memo') {
  applyData(await window.api.createTab(folder, defaultTabSetting(name, type)))
